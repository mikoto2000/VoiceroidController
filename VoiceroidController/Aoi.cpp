...
const _TCHAR* Aoi::SAVE_DIALOG_NAME = _T("音声ファイルの保存");
const _TCHAR* Aoi::SAVING_DIALOG_NAME = _T("音声保存");
const _TCHAR* Aoi::PLAY_BUTTON_NAME = _T(" 再生");

void Aoi::echo(std::string sentence, BOOL isSync)
{
	HWND textArea = Aoi::SearchTextArea(aoi);
	...
	// "再生" ボタンを探す
	HWND play_button = SearchPlayButton(aoi);
...
void Aoi::save(std::string sentence, std::string outFilePath, BOOL isSync)
{
	HWND textArea = Aoi::SearchTextArea(aoi);
	...
	// "音声保存ボタン" を探して押下
	HWND save_button = SearchSaveButton(aoi);
	PostMessage(save_button, BM_CLICK, 0, 0);
	Sleep(WAIT_TIME);

	// "音声ファイルの保存" ウィンドウを探す
	SearchHwndParam ssdp;
	EnumWindows(Aoi::SearchSaveDialog, (LPARAM)&ssdp);
...
BOOL CALLBACK Aoi::SearchSaveDialog(HWND hwnd, LPARAM lp) {
...
BOOL CALLBACK Aoi::SearchSavingDialog(HWND hwnd, LPARAM lp) {
	SearchHwndParam* ssdp = (SearchHwndParam*)lp;
...
BOOL CALLBACK Aoi::SearchConfirmationOverwriteDialog(HWND hwnd, LPARAM lp) {
...
HWND CALLBACK Aoi::SearchTextArea(HWND mainWindow)
{
...
HWND CALLBACK Aoi::SearchPlayButton(HWND mainWindow) {
	HWND tmp = GetWindow(mainWindow, GW_CHILD);
	tmp = GetWindow(tmp, GW_CHILD);
	tmp = GetWindow(tmp, GW_HWNDNEXT);
	tmp = GetWindow(tmp, GW_CHILD);
...
HWND CALLBACK Aoi::SearchStopButton(HWND mainWindow) {
	HWND tmp = GetWindow(mainWindow, GW_CHILD);
	tmp = GetWindow(tmp, GW_CHILD);
	tmp = GetWindow(tmp, GW_HWNDNEXT);
	tmp = GetWindow(tmp, GW_CHILD);
...
HWND CALLBACK Aoi::SearchSaveButton(HWND mainWindow) {
	HWND tmp = GetWindow(mainWindow, GW_CHILD);
	tmp = GetWindow(tmp, GW_CHILD);
	tmp = GetWindow(tmp, GW_HWNDNEXT);
	tmp = GetWindow(tmp, GW_CHILD);
...
HWND CALLBACK Aoi::SearchSaveFilePath(HWND saveDialog) {
...
HWND CALLBACK Aoi::SearchSaveButtonInSaveDialog(HWND saveDialog) {
	HWND tmp = GetWindow(saveDialog, GW_CHILD);
	tmp = GetWindow(tmp, GW_HWNDNEXT);
	tmp = GetWindow(tmp, GW_HWNDNEXT);
...
HWND CALLBACK Aoi::SearchYesButtonInConfirmationOverwriteDialog(HWND confirmationOverwriteDialog) {
